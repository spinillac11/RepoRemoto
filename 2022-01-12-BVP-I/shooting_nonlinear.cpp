...
int main(void)
{
  ...
  auto faux = [&y](double z){ //Como X0, XF y DX estan definidas globalmente no hay que capturarlas
    y[0] = T0; y[1] = z;
    boost::numeric::odeint::integrate(fderiv, y, X0, XF, DX); // No recive funcion para imprimir
    return y[0] - TF;};

  // perform a Newton-Raphson procedure to find the root
  // compute numerically the derivative
  int nsteps = 0;
  double z0  = newton(-10, faux, 0.001, nsteps); //Valor de Z para el cual se cumple que Y[0] - TF = 0
