...
#include <cmath>
#include <numeric>
#include <valarray> // Resolver problemas vectoriales, operaciones componente a componente
#include <cstdlib>

// constants
const double W = 3.1237;

typedef std::valarray<double> state_t; // defino el vector con valarray y le doy un alias
...
void integrate_euler(deriv_t deriv, system_t & y, double tinit, double tend, double dt, printer_t writer)
{
  ...
  for(int ii = 0; ii < nsteps; ++ii) {
    time = 0.0 + ii*dt;
    deriv(y, dydt, time);
    y += dydt*dt; // Ya no es necesario el bucle for 
    print(y, time);
  }
}
